...
- New FreeRTOS phase: shift from FreeRTOS-inspired C models to real FreeRTOS API muscle memory. The model layer should now be used only as a quick explanation aid when the real API is confusing.
- New lesson pattern for RTOS topics: state the embedded/OBC problem, define the module boundary, write testable core logic where possible, implement the real FreeRTOS version, run on target or a real FreeRTOS simulator/port when available, then review failure modes.
- Queue lessons should drill real names and return values: `QueueHandle_t`, `xQueueCreate`, `xQueueSend`, `xQueueReceive`, `xQueueSendFromISR`, `pdPASS`, `pdFAIL`, `BaseType_t`, `TickType_t`, and `portMAX_DELAY`.
- Keep reinforcing queue copy semantics: `&sample` is the source address copied into the queue, `&received` is caller-provided destination storage, and queues copy bytes unless the item type is deliberately a pointer with a designed lifetime/ownership rule.
- Lessons 0016-0020 were scaffolded as a forward arc without deleting or renaming the existing 0014/0015 queue lessons: 0016 real ISR-to-task queue handoff, 0017 periodic task timing, 0018 FreeRTOS toolbox decisions, 0019 module/task/queue/app boundaries, and 0020 mini OBC integration slice. Learning records for these should wait until the user completes or reviews the exercises.
- Architecture/process lessons should cite the specific F Prime development, ports/components/topologies, unit testing, F Prime style, cFE application guide, and Power of Ten resources rather than relying on generic architecture advice.
